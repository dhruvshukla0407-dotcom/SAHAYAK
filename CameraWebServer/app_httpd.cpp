...
static esp_err_t identity_handler(httpd_req_t *req) {
  ...
  int len = snprintf(
    ...
    "{\"deviceId\":\"%s\",\"stationConnected\":%s,\"stationIp\":\"%u.%u.%u.%u\",\"apIp\":\"%u.%u.%u.%u\",\"capturePath\":\"/capture\",\"streamPath\":\"/stream\",\"streamPort\":81,\"wsPath\":\"/ws\"}",
    getDeviceId(),
  ...
  if (len <= 0 || len >= (int)sizeof(json_response)) {
    httpd_resp_send_500(req);
    return ESP_FAIL;
  }
